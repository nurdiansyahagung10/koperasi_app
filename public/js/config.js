...
const apiRoutes = {
    ...
    resorts: `/api/v1/resorts/branch_office/${
        window.location.href.split("/")[5]
    }/resort`,
    detailresorts: `/api/v1/detailresorts/resort/${
        window.location.href.split("/")[5]
    }/detailresort`,
};
...
const extra_function_for_body_table = (mindata, maxdata) => {
    function hideOtherDropdowns(currentIndex) {
        for (let j = mindata; j <= maxdata; j++) {
            if (j != currentIndex) {
                const anotherdropdown = document.getElementById(
                    `dropdown-${j}`
                );
                if (anotherdropdown) {
    ...
    const fetchDeleteHandler = (link, name) => {
        for (let i = mindata; i <= maxdata; i++) {
            ...
            const modal_button_hidden = document.getElementById(
                `modal-button-hidden-${i}`
            );
            const final_choices_delete = document.getElementById(
            ...
            if (final_choices_delete) {
                final_choices_delete.addEventListener(
                    "click",
                    async (event) => {
    ...
    function set_permissions() {
        for (let i = mindata; i <= maxdata; i++) {
            ...
            if (checkbox_permissions) {
                checkbox_permissions.addEventListener(
                    "change",
                    async (event) => {
    ...
    switch (path) {
        ...
        case "pdls":
            fetchDeleteHandler(
                `/${path}/branch_employee/${window.location.href
                    .split("/")
                    .slice(5, 6)
                    .join("/")}/`,
                path.split("_").join(" ")
