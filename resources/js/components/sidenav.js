...
const section_main_dashboard = document.getElementById(
...
const sidenav_button = document.querySelectorAll(".sidenav-button");
...
sidenav_button.forEach((item) => {
    item.addEventListener("click", (event) =>
        event.target.closest("button").name == ""
            ? sidenav_triger(event)
            : sidenavdrawer(event, event.target.closest("button").name)
    );
});

function sidenav_triger(event) {
