    ...
    <section id="section-roles-dashboard " class="flex-1">
        ...
        <div class="w-full  flex h-full justify-center items-center">
            ...
            <form method="POST" id="formpost" enctype="multipart/form-data" class="w-full flex flex-col gap-4 mt-2.5 "
                action="{{ route('branch_employees.update', ['role' => $role, 'branch_employee' => $branch_employee->id]) }}">
                ...
                <div class="button  bg-white border flex items-center px-4 py-1.5 rounded-xl ">
                    <div class="flex flex-col w-full">
                        ...
                        <select required name="head_id" id="head_office"
                            ...
                            <option value="{{ $branch_employee->head_office->id }}">
                                {{ $branch_employee->head_office->province }}
                            </option>
                            @foreach ($head_offices as $item)
                                @if ($item->province != $branch_employee->head_office->province)
                ...
                <div class="button bg-white border flex items-center px-4 py-1.5 rounded-xl ">
                    <div class="flex flex-col w-full">
                        ...
                        <textarea required name="hometown" id="hometown"
                            class="w-full outline-hidden h-32 text-sm border-none shadow-none ">{{ $branch_employee->hometown }}</textarea>
                    </div>
                </div>
                <div class="button  bg-white border flex items-center px-4 py-1.5 rounded-xl ">
                    <div class="flex flex-col w-full">
    ...
    <script>
        ...
        `;
                ...
                data.body.forEach(item => {
                    ...
                    if (item.id == "{{ $branch_employee->branch_id }}") {
                        branch_office.innerHTML = `
